...
namespace Drag_AND_Drop_between_Forms
{
    ...
    {
        public Func<Double,Double,Double,Double> Segunda_equipo24_sCO2;

        public int generarecuaciones(int contecuaciones1 ,Double TipodeEquipo, Double N1, Double N2, Double N3, Double N4, Double D1, Double D2, Double D3, Double D4, Double D5, Double D6, Double D7, Double D8, Double D9, Double adicional1, Double adicional2, Double adicional3, Double adicional4)
        {
            //Creamos las ecuaciones del Equipo TIPO 1: CONDICIÓN DE CONTORNO para Tablas de Agua-Vapor ASME 1967
            if (TipodeEquipo == 1)
            {
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
            ...
            else if (TipodeEquipo == 22)
            {
             ...
             //Creamos las ecuaciones del Equipo TIPO 23: INTERCAMBIADOR igual que el tipo 22 pero para s-CO2
            else if (TipodeEquipo == 23)
            {
                Parameter W1 = new Parameter();
                Parameter W2 = new Parameter();
                Parameter W3 = new Parameter();
            ...
            //-------------------------------------------------------------------------------------------------------------------------------
            //Creamos las ecuaciones del Equipo TIPO 24: TURBINA supercritica SIN PÉRDIDAS EN EL ESCAPE con fluido Supercrítico
            else if (TipodeEquipo == 24)
            {
                Parameter W1 = new Parameter();
                Parameter W2 = new Parameter();
                Parameter P1 = new Parameter();
                ...
                if (D1 != 0)
                {
                    //Func<Double> segundaecuacion = () => H2 - H1 + D1 * (H1 - acceso.hps(P2, (acceso.sph(P1, H1))));
                    Func<Double> segundaecuacion = () =>
                    {
                         //Convert Units: From British: W (Lb/sg), P (psia), H (BTU/Lb) To SI: W (kg/sg), P(bar), H (kJ/kg) and after to P(kPa), H(kJ/mol) and s(kJ/mol K)
                         Double wmm, enthalpy1,pressure1,entropy1,pressure2,enthalpy2;
                         wmm= luis1.working_fluid.MolecularWeight;
                         pressure1 = P1.Value * 0.0689476;//convert bar to psia
                         enthalpy1 = H1.Value * 2.326; //convert to BTU/lb
                         luis1.working_fluid.FindStatueWithPH(pressure1*100, enthalpy1*wmm); //With Pin,Hin finds Sin
                         entropy1 = luis1.working_fluid.Entropy;//obtaining Sin
                         entropy1 = entropy1 * wmm;
                         pressure2 = P2.Value * 0.0689476; //convert psi to bar
                         luis1.working_fluid.FindStatueWithPS(pressure2*100, entropy1); //With Pout,Sin finds Hout(enthalpy2)
                         enthalpy2 = luis1.working_fluid.Enthalpy;
                         enthalpy2 = 0.429923 * enthalpy2; //convert BTU/lb to kJ/kg
                         return (H2 - H1 + D1 * (H1 - enthalpy2));
                    };
                ...
                Func<Double> terceraecuacion = () => P2 - D2;
                functions.Add(terceraecuacion);
            ...
            //-------------------------------------------------------------------------------------------------------------------------------
            //Creamos las ecuaciones del Equipo TIPO 25: COMPRESOR supercritico
            else if (TipodeEquipo == 25)
            {
                Parameter W1 = new Parameter();
                Parameter W2 = new Parameter();
                Parameter P1 = new Parameter();
