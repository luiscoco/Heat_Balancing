...
namespace Arrows
{
    ...
    public class ArrowRenderer
    {
        ...
        public void DrawArrow(Graphics g, Pen penArrow, Pen penLine, Brush brush, float x1, float y1, float x2, float y2, bool arrowsAtBeginingAndAtEnd)
        {
            DrawArrow(g, penArrow, penLine, brush, new PointF(x1, y1), new PointF(x2, y2), arrowsAtBeginingAndAtEnd);
        }
        ...
        /// <param name="penArrow">The penArrow of the stroke.</param>
        /// <param name="brush">The brush of the fill.</param>
        /// <param name="p1">Initial point.</param>
        /// <param name="p2">Terminal point.</param>
        public void DrawArrow(Graphics g, Pen penArrow, Pen penLine, Brush brush, PointF p1, PointF p2, bool arrowsAtBeginingAndAtEnd)
        {
            if (arrowsAtBeginingAndAtEnd == true)
            {
                PointF[] aptArrowHead = new PointF[3];
                PointF[] aptArrowHead1 = new PointF[3];

                //Set first Arrow node to terminal point            
                aptArrowHead[0] = p2;
                ...
                float th = Width / (3.0f * lineLength);
                float ta = Width / (3.0f * ((float)Math.Tan(Theta / 2.0f)) * lineLength);

                // Arrow drawing, set first node to start point            
                aptArrowHead1[0] = p1;

                // find the base of the arrow
                PointF pBase1 = new PointF(aptArrowHead1[0].X + ta * vecLine[0], aptArrowHead1[0].Y + ta * vecLine[1]); //base of the arrow

                // build the points on the sides of the arrow
                aptArrowHead1[1] = new PointF(pBase1.X + th * vecLeft[0], pBase1.Y + th * vecLeft[1]);
                aptArrowHead1[2] = new PointF(pBase1.X + -th * vecLeft[0], pBase1.Y + -th * vecLeft[1]);

                if (FillArrowHead) g.FillPolygon(brush, aptArrowHead1); //fill arrow head if desired

                g.DrawPolygon(penArrow, aptArrowHead1); //draw arrow at the begining of the connection 
                ...
                g.DrawLine(penLine, pBase1, pBase); //master line

                if (FillArrowHead) g.FillPolygon(brush, aptArrowHead); //fill arrow head if desired

                g.DrawPolygon(penArrow, aptArrowHead); //draw arrow at the connection end
            }

            else if (arrowsAtBeginingAndAtEnd == false)
            {
                PointF[] aptArrowHead = new PointF[3];
                ...
                g.DrawLine(penLine, p1, pBase); //master line
        ...
        public void DrawArrows(Graphics g, Pen penArrow, Pen penLine, Brush brush, PointF[] apf)
        {
            DrawArrows(g, penArrow, penLine, brush, apf, 0, 0);
        }

        public void DrawArrows(Graphics g, Pen penArrow, Pen penLine, Brush brush, PointF[] apf, float startOffset, float endOffset)
        {
            ...
            for (int i = 0; i < newApf.Length - 1; i++)
            {
                ...
                DrawArrow(g, penArrow, penLine, brush, nFromP, nToP, false);
                fromP = toP;
            }
        }
        ...
        public void DrawArrows(Graphics g, Pen penArrow, Pen penLine, Brush brush, PointF[] apf, float[] startOffsets, float[] endOffsets)
        {
            if (apf.Length < 2) throw new ArgumentException("PointF[] apf has to be at least 2 in length");
