...
namespace Dalssoft.DiagramNet
{
	...
	{
		internal protected LineElement[] lines = {new LineElement(0,0,0,0,true)};
		...
		public RightAngleLinkElement(ConnectorElement conn1, ConnectorElement conn2, bool ArrowInConnections, Color ArrowFillcolor, bool fillArrowsWithColor1, int ArrowWith1, float ArrowAngle1, Color linesBorderColor1, Color arrowsBorderColor1, float arrowsBorderWidth1, float linesBorderWidth1, bool arrowsAtBeginingAndAtEnd1) : base(conn1, conn2)
		{
			needCalcLink = true;

            arrowsAtBeginingAndAtEnd = arrowsAtBeginingAndAtEnd1;

            arrowsBorderWidth = arrowsBorderWidth1;
            linesBorderWidth = linesBorderWidth1;

            ArrowWith = ArrowWith1;
            ArrowAngle = ArrowAngle1;

            fillArrowsWithColor = fillArrowsWithColor1;

            arrowColor = ArrowFillcolor;
            linesBorderColor = linesBorderColor1;
            arrowsBorderColor = arrowsBorderColor1;

            fillArrow = ArrowInConnections;

            linetypearrow = ArrowInConnections;
        ...
        internal override void Draw(Graphics g)
        {
            ...
            {
               lines[i].color = arrowColor;
               lines[i].borderColorArrow = arrowsBorderColor;
               lines[i].borderColorLine = linesBorderColor;
               lines[i].borderColorArrow = arrowsBorderColor;
               lines[i].fillArrow = fillArrow;
               lines[i].arrowWith = ArrowWith;
               lines[i].arrowAngle = ArrowAngle;
               lines[i].borderWidthArrow = arrowsBorderWidth;
               lines[i].borderWidthLine= linesBorderWidth;
               lines[i].arrowsAtBeginingAndAtEnd = arrowsAtBeginingAndAtEnd;
               lines[i].Draw(g);
            }
        }
		...
		private void InitConnectors(ConnectorElement conn1, ConnectorElement conn2)
		{
				...
				)
			{
                ...
                lines[0] = new LineElement(0, 0, 0, 0, linetypearrow);
                lines[1] = new LineElement(0, 0, 0, 0, linetypearrow);
			}
			else	
			{
                ...
                lines[0] = new LineElement(0, 0, 0, 0, linetypearrow);
                lines[1] = new LineElement(0, 0, 0, 0, linetypearrow);
                lines[2] = new LineElement(0, 0, 0, 0, linetypearrow);
			}
